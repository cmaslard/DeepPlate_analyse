    ...
    )
{
  ...
  if(grp_var==""){
    if(Xlab=="condition"){
      ...
      Z=Z%>% mutate(group_cat_var=factor(group_cat_var,levels=c("SS_WT1", "SS_W78*", "SS_WT2", "SS_E568K", "SD_WT1", "SD_W78*", "SD_WT2", "SD_E568K")))
    }
    ...
    if(Xlab=="sulfur_condition"){
      Z=Z%>% mutate(group_cat_var=factor(group_cat_var,levels=c("SS","SD")))
    }
    if(Xlab=="genotype"){
      Z=Z%>% mutate(group_cat_var=factor(group_cat_var,levels=c("KAY", "WT1", "W78*", "WT2", "E568K")))
    }
    if(Xlab=="geno_depod"){
      Z=Z%>% mutate(group_cat_var=factor(group_cat_var,levels=c("WT1_All pod","W78*_All pod","WT2_All pod","E568K_All pod",
    ...
    p = p+ #facet_grid(.~ facet)
      ggh4x::facet_wrap2(
        ...
        strip  = ggh4x::strip_themed(
          background_x = ggh4x::elem_list_rect(fill = Z[1,strip_fill_vec]),
          text_x       = ggh4x::elem_list_text(colour = Z[1,strip_text_vec])
        )
      )
  }
