    ...
    )
{
  ...
  row.names(Z)=as.data.frame(Z[,label_outlier])[,1] #avant label outlier
  dat <- Z %>% tibble::rownames_to_column(var="outlier") %>% dplyr::group_by(compile_cond) %>% mutate(is_outlier=ifelse(is_outlier(get(Y_bis)), Y, as.numeric(NA)))
  dat$outlier[which(is.na(dat$is_outlier))] <- as.numeric(NA)
  Z$outlier=dat$outlier
